...
namespace Ediux.HomeSystem.SystemManagement
{
    public interface ISystemMessageAppService : ICrudAppService<SystemMessageDto, Guid, AbpSearchRequestDto>, ITransientDependency
    {
        ...
        /// <param name="logLevel">操作紀錄層級(預設是none)</param>
        /// <returns>成功建立的系統訊息</returns>
        Task<SystemMessageDto> CreateSystemMessageAsync(Guid userId, string message, bool sendMail = false, bool push = false, ILogger logger = null, LogLevel logLevel = LogLevel.None);
        ...
        /// 回覆訊息
        /// </summary>
        /// <param name="originalMessage">原始內容</param>
        /// <param name="message">回覆內容</param>
        /// <param name="sendMail">是否發送為電子郵件通知</param>
        /// <param name="push">是否要發送推播</param>
        /// <returns></returns>
        Task<SystemMessageDto> ReplyMessageAsync(SystemMessageDto originalMessage, string message, bool sendMail, bool push);
    }
}
