	...
	$.extend(
		altEditor.prototype,
		{
			...
			_openEditModal: function () {
				...
				for (var i in dt.context[0].aoColumns) {
					...
					columnDefs[i] = {
						...
						upload: (obj.upload ? obj.upload : false),
						accept: (obj.accept ? obj.accept : false)
					};
				}
				var adata = dt.rows({
					selected: true
				...
				data += "<form name='altEditor-form' role='form' novalidate enctype='multipart/form-data'>";
				...
				for (var j in columnDefs) {
					...
					if (columnDefs[j].type.indexOf("hidden") >= 0) {
						...
						if (columnDefs[j].type.indexOf("file") >= 0) {
							data += "<input type='file' id='";

							if (!columnDefs[j].upload) {
								data += that._quoteattr(columnDefs[j].name);
							} else {								
								data += that._quoteattr(columnDefs[j].upload);
							}
							...
							data += "' data-errorMsg='"
								...
								+ "' "
								+ (columnDefs[j].maxLength == false ? "" : " maxlength='" + columnDefs[j].maxLength + "'")
								+ (columnDefs[j].accept ? " accept='" + columnDefs[j].accept + "'" : "")
								+ " style='overflow:hidden' class='form-control' value=''>";
				...
				for (var j in columnDefs) {
					...
					if (columnDefs[j].type === 'file') {
						var fileinputid = '';

						if (!columnDefs[j].upload) {
							fileinputid = columnDefs[j].name;
						} else {
							fileinputid = columnDefs[j].upload;
						}

						const fileUploader = document.querySelector('#' + fileinputid);

						fileUploader.addEventListener('change', (e) => {
							that.formData.append(fileinputid, e.target.files[0]);
						});
					}
				}
			...
			_editRowData: function () {
				...
				$('form[name="altEditor-form"] *').filter(':input').each(function (i) {
					...
					if ($(this).attr('type') !== 'file') {
						...
						if (typeof ($(this).val()) === 'undefined' || $(this).val() == null) return;
						if ($(this).val().replace(/\s/g, '').length < 1) return;

						that.formData.append($(this).attr('id'), $(this).val());
					}
				});
			...
			_openAddModal: function () {
				...
				// Building add-form
				var data = "";
				data += "<form name='altEditor-form' role='form' class='needs-validation' novalidate='' enctype='multipart/form-data'>";
				for (var j in columnDefs) {
					if (columnDefs[j].type.indexOf("hidden") >= 0) {
